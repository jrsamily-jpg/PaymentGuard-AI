...
def theme():
    ...
    active_art = SIGNIN_ART if st.session_state.get("workspace_page") == "Data & access" and not st.session_state.get("workspace_token") else ART
    st.markdown(f"<style>:root{{--pg-art:url(/app/static/{active_art.name});}}{css}</style>", unsafe_allow_html=True)
...
def hero(title, subtitle, kicker="PAYMENT RISK INTELLIGENCE"):
    ...
    if overview:
        markup = f'<div class="hero" style="background-image:linear-gradient(90deg,rgba(9,17,25,.28),transparent),url(data:image/png;base64,{artwork()})"><div class="hero-content"><div class="eyebrow">{escape(kicker)}</div><h1>{title_html}</h1><p>{escape(subtitle)}</p></div><span class="hero-caption">PAYMENTGUARD / INTELLIGENCE IN FOCUS</span></div>'
    else:
        page = st.session_state.get("page", title)
        markup = f'<div class="page-heading"><div><div class="eyebrow">PAYMENTGUARD / WORKSPACE</div><h1>{escape(page)}</h1><p>{escape(subtitle)}</p></div><img class="mini-art" src="data:image/png;base64,{artwork()}" alt="" aria-hidden="true"></div>'
...
def chart(fig, height=290):
    fig.update_layout(
        ...
        font=dict(family="Arial, sans-serif", color="#afbfcb", size=12),
        height=height,
        margin=dict(l=12, r=16, t=24, b=25),
        colorway=COLORS,
